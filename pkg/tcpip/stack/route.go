...
func (r *Route) ResolvedFields(afterResolve func(RouteInfo, bool)) (RouteInfo, <-chan struct{}, *tcpip.Error) {
	r.mu.RLock()
	...
	linkAddr, ch, err := r.outgoingNIC.getNeighborLinkAddress(nextAddr, linkAddressResolutionRequestLocalAddr, r.linkRes, func(linkAddr tcpip.LinkAddress, success bool) {
		if afterResolve != nil {
			if success {
				afterResolveFields.RemoteLinkAddress = linkAddr
			}

			afterResolve(afterResolveFields, success)
		}
	})
	if err == nil {
