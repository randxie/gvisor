...
type errorCapturingLimitedReader struct {
	R   io.Reader
	N   int64
	err error
}

func (e *errorCapturingLimitedReader) Len() int {
	return int(e.N)
}

func (e *errorCapturingLimitedReader) Read(p []byte) (int, error) {
	// NB: this part could be implemented by composing io.LimitedReader, but the
	// behavior is slightly different; io.LimitedReader would skip the Read call
	// when N == 0. This more closely matches the previous implementation. It
	// might be possible to compose io.LimitedReader instead.
	if int64(len(p)) > e.N {
		p = p[:e.N]
	}
	n, err := e.R.Read(p)
...
func (s *SocketOperations) ReadFrom(ctx context.Context, _ *fs.File, r io.Reader, count int64) (int64, error) {
	...
	if err == tcpip.ErrWouldBlock {
		...
		return n, f.err // Propagate error.
	}
