...
//
// TODO(gvisor.dev/issue/5264): we don't support OFD locks
// through fcntl, which would return a struct with pid = -1.
func (l *Locks) TestRegion(ctx context.Context, uid UniqueID, t LockType, r LockRange) linux.Flock {
	var f linux.Flock
	f.Type = linux.F_UNLCK
	switch t {
	case ReadLock:
		l.testRegion(r, &f, func(lock Lock) bool {
	...
	case WriteLock:
		l.testRegion(r, &f, func(lock Lock) bool {
			if lock.Writer == nil {
				for k, v := range lock.Readers {
					if k != uid {
						...
						// TODO(deandeng): convert based on pidns with (*kernel.Task).PIDNamespace().IDOfTask((*kernel.Task).tg.pidns.owner.Root.TaskWithID(t.tg))
						// We can't do this because of circular package dependencies.
						f.PID = v.PID
						return false
					}
				}
			...
			f.Type = linux.F_WRLCK
			f.PID = lock.WriterInfo.PID // TODO(deandeng): convert based on ns
			return false
		})
	default:
...
func (l *Locks) testRegion(r LockRange, f *linux.Flock, check func(lock Lock) bool) {
	l.mu.Lock()
	defer l.mu.Unlock()
	...
	for seg.Ok() && seg.Start() < r.End {
		...
		if !check(value) {
			// Stop at the first conflict detected.
			f.Start = int64(seg.Start())
			f.Len = int64(seg.End()) - int64(seg.Start())
			return
		}
		seg = seg.NextSegment()
	}
