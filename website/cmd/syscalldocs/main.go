...
// syscallDocURL returns a doc url for a given syscall, doing it's best to return a url that exists.
func syscallDocURL(name string) string {
	...
	if url, ok := customDocs[name]; ok {
		...
		return "https://man7.org/linux/man-pages/man2/syscalls.2.html"
	}
