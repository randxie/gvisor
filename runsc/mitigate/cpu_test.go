...
func TestReadFile(t *testing.T) {
	...
	for _, c := range set {
		t.Logf("CPU: %+v: %t", c, c.isVulnerable(nil))
	}
}
...
power management:`
	...
	for _, tc := range []struct {
		...
		for _, c := range set {
			if result := c.isVulnerable(func(other *cpu) bool {
				return cascadeLake.similarTo(other)
			}); result != tc.vulnerable {
