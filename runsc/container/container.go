...
func (c *Container) populateStats(event *boot.EventOut) {
	...
	// only counting Running contianers.
	var containerUsage uint64
	var sentryUsage uint64
	...
	if err != nil {
		...
		log.Warningf("Error getting cgroup CPU usage for container: %v", err)
		event.Event.Data.CPU.Usage.Total = containerUsage
		return
	}
	...
	if sentryUsage == 0 {
		log.Warningf("No sentry usage reported")
		sentryUsage = cgroupsUsage
	...
	event.Event.Data.CPU.Usage.Total = uint64(float64(containerUsage) * (float64(cgroupsUsage) / float64(sentryUsage)))
	return
}
